...
class PaginatorContext(object):
    ...
    def sort(self, request, session_prefs=None):
        ...
        sort = None
        sticky = request.user.is_authenticated() 
        # and request.user.prop.preferences and request.user.prop.preferences.get('sticky_sorts', False)
    ...
    if sort:
        def sort_tabs():
            ...
            for name, label, url, descr in sorts:
                paginator.__dict__['%s_sort_link' % name] = smart_text(url)
