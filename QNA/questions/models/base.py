...
class CachedQuerySet(models.query.QuerySet):
    ...
    def iterator(self):
        ...
        if (not to_return) and key_list:
            ...
            to_return = [
                (ck in cached) and self.obj_from_datadict(cached[ck]) or ToFetch(force_text(key_list[i])) for i, ck in enumerate(row_keys)
            ]

            if len(cached) != len(row_keys):
                to_fetch = [str(tr) for tr in to_return if isinstance(tr, ToFetch)]

                fetched = dict([(force_text(r.__dict__[on_cache_query_attr]), r) for r in
    ...
    def _get_query_hash(self):
        try:
            return md5(str(self.query).encode("utf-8")).hexdigest()
        except:
            try:
